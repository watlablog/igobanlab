...
`勢力表示` はローカル推定器で実行されるため API へは接続しません。

## 勢力図 v2 仕様

- `勢力表示` はブラウザ内のローカル Sabaki 推定器を実行します（ネットワーク不要）。
- 推定器は `@sabaki/influence` を主軸に、`@sabaki/deadstones` の軽量補正を併用します。
- API（`/v1/analyze/score`）は勢力表示では使用せず、候補手評価用途に維持します。
...
- 勢力値 `v` は 3 段階に量子化して表示します。
  ...
  - `0.33 <= |v| < 0.66`: 中
  - `|v| >= 0.66`: 強
- 解析結果カードには `source`（`sabaki-local`）と `elapsed`（ms）を表示します。
- 死石予測は盤面に `×` オーバーレイ表示できます（解析カードの `死石表示 ON/OFF` で切替）。
- `influence B/W/N` は石数ではなく、上記閾値で分類した勢力マス数の集計です。
- ローカル推定器が失敗した場合は、エラーをそのままUIに表示します。

## SGF 読み込み仕様

- 盤面画面の `Import SGF` ボタンから `.sgf` ファイルを読み込みます。
- 読み込み対象:
  - 9/13/19路盤
  - メインライン手順（分岐がある場合は先頭分岐を採用）
  - `PB/PW/BR/WR/DT/PC` の対局情報
- 未対応:
  - 圧縮座標（例: `AB[aa:cc]`）
  - 9/13/19 以外の盤サイズ
...
- 原因: `候補手評価` のAPI呼び出しが分析APIではなくHTMLを受け取っている（`VITE_API_BASE_URL` 未設定 or 誤設定）
- 対処:
...
- エラー: `Failed to fetch`
- 主な原因:
  - `候補手評価` の Cloud Run APIに到達できないURLを指定している
  - `候補手評価` の CORS未設定でブラウザが遮断している
  - `候補手評価` で `http://` のAPI URLを `https://` ページから呼んでいる（Mixed Content）
- 対処:
